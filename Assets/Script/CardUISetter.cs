    ...
    // 덱 창과 무덤 창도 일단은 여기서 관리하는데 분리할수도 있고 아닐수도 있고..
{
    ...
    [SerializeField] private RectTransform AttackMiddleZone;//중앙에 있는 공격카드 위치
    ...
    CardView attackCard;//중앙에 있는 공격카드 표현
    ...
    void e_CardLeftClicked(EventBus.CardLeftClickedEvent e)//이벤트 버스에서 구독함.invoke() 하면 실행됨 연출 관련 해서만 적어두자.
    {
        ...
        //그리고 여기에다간 플레이어 방어존 카드가 눌린건지 확인을 위해 코드를 짜면 될 것 같다.
        //그리고 여기에다간 플레이어 방어존 카드가 눌린건지 확인을 위해 코드를 짜면 될 것 같다.
        //그리고 여기에다간 플레이어 방어존 카드가 눌린건지 확인을 위해 코드를 짜면 될 것 같다.
        //그리고 여기에다간 플레이어 방어존 카드가 눌린건지 확인을 위해 코드를 짜면 될 것 같다.
        //그리고 여기에다간 플레이어 방어존 카드가 눌린건지 확인을 위해 코드를 짜면 될 것 같다.
